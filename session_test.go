...
func TestPty(t *testing.T) {
	...
	winWidth := 40
	winHeight := 80
	termModes := make(gossh.TerminalModes)
	ttyOPOSPEED := uint32(38400)
	termModes[gossh.TTY_OP_OSPEED] = ttyOPOSPEED
	done := make(chan bool)
	session, _, cleanup := newTestSession(t, &Server{
		Handler: func(s Session) {
			...
			mode := ptyReq.Termmodes[gossh.TTY_OP_OSPEED]
			if ptyReq.Termmodes[gossh.TTY_OP_OSPEED] != ttyOPOSPEED {
	...
	defer cleanup()
	if err := session.RequestPty(term, winHeight, winWidth, termModes); err != nil {
		t.Fatalf("expected nil but got %v", err)
	}
